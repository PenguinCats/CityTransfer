...
from utility.args_parser_remove_transfer import parse_args
from utility.log_helper import logging, logging_config
from utility.data_remove_transfer import DataLoader
from utility.metrics import ndcf_at_k, ndcf_at_k_test
from utility.visualization_trick_feature_removed import VisualizationTool
from CityTransfer_remove_transfer import CityTransfer
...
if __name__ == '__main__':
    ...
    target_batch = [data.target_train_grids[i: i + args.batch_size]
                    for i in range(0, len(data.target_train_grids), args.batch_size)]
    all_batch = [data.all_grids[i: i + args.batch_size]
                 for i in range(0, len(data.all_grids), args.batch_size)]
    while len(target_batch) < len(all_batch):
        random_i = random.randint(0, len(data.target_train_grids) - args.batch_size)
        target_batch.append(data.target_train_grids[random_i: random_i + args.batch_size])
    ...
    for epoch in range(args.n_epoch):
        model.train()
        iter_total_loss = 0
        for batch_iter in range(len(all_batch)):
            time_iter = time()
            ...
            batch_all_index = all_batch[batch_iter]

            # Auto Encoder
            target_grid_feature = data.feature[:, batch_target_index]
            all_grid_feature = data.feature[:, batch_all_index]

            ae_target_batch_loss = args.lambda_3 * model('cal_auto_encoder_loss', target_grid_feature)
            ae_all_batch_loss = args.lambda_3 * model('cal_auto_encoder_loss', all_grid_feature)

            batch_total_loss += ae_target_batch_loss + ae_all_batch_loss

            # Prediction Model
            feature_target, score_target = data.get_feature_and_rel_score_for_prediction_model(batch_target_index, 't')
            feature_all, score_all = data.get_feature_and_rel_score_for_prediction_model(batch_all_index, 'a')

            prediction_all_batch_loss = model('cal_prediction_loss', data.portion_enterprise_index,
                                              batch_all_index, feature_all, score_all)
            prediction_target_batch_loss = model('cal_prediction_loss', data.target_enterprise_index,
                                                 batch_target_index, feature_target, score_target)

            batch_total_loss += prediction_all_batch_loss + args.lambda_1 * prediction_target_batch_loss
            ...
            if DEBUG and (batch_iter % args.print_every) == 0:
                logging.info('Training: Epoch {:04d} / {:04d} | Iter {:04d} / {:04d} | Time {:.1f}s '
                             ...
                             format(epoch, args.n_epoch, batch_iter, len(all_batch) - 1, time() - time_iter,
    ...
    with torch.no_grad():
        ...
        real_rank = data.target_test_grids[real_rank]
